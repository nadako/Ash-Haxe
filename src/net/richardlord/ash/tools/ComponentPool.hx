...
class ComponentPool
{
    private static var pools:ObjectHash<Class<Dynamic>, Array<Dynamic>> = new ObjectHash<Class<Dynamic>, Array<Dynamic>>();
 ...
 */
    ...
    public static function get<TComponent>(componentClass:Class<TComponent>):TComponent
    {
        ...
        if (pool.length > 0)
        {
            trace("Getting " + componentClass + " from pool");
            return pool.pop();
        }
        else
        {
            trace("Creating " + componentClass);
